...
export function createChemShader() {
  return /* glsl */`
    ...
    uniform float phaseReferenceOffset;
    uniform float angleToRadians;
    ...
    void main(){
      ...
      if (i < N){
        ...
        float dphi = helixHandedness * qPitch * ds * angleToRadians + c.w;
  ...
  return /* glsl */`
    ...
    void main(){
      ...
      if (i >= idxRung0 && i < idxRung0 + rungCount) {
        ...
        float dThetaAxial = helixHandedness * qP * axialShift * angleToRadians;

        // MDPI Eq. (12): Strand A and B with phase offset
        float phiA = phi + phaseReferenceOffset + strandAPhaseOffset - 0.5 * gapPhase;
        float phiB = phi + phaseReferenceOffset + strandBPhaseOffset + dThetaAxial + 0.5 * gapPhase;
