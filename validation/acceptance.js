...
function evaluateInvariants({ posPixels, chemPixels, extPixels, constants, criteria, zipMode, scenarioName, emEnabled }) {
  ...
  const topologyMinRatioUnzipped = criteria?.topologyAndRoutingMinRatioUnzipped ?? 0.1;
  const relaxedTopologyScenario = scenarioName === 'unzip' || scenarioName === 'reroute';
  const helixRequiredRatio = relaxedTopologyScenario ? topologyMinRatioUnzipped : topologyMinRatio;
  const rungRequiredRatio = relaxedTopologyScenario ? topologyMinRatioUnzipped : topologyMinRatio;
  const phaseRequiredRatio = topologyMinRatio;
  const hubRequiredRatio = topologyMinRatio;
  ...
  const metrics = {
    helixRadiusTolerance: { pass: radiusPass, total: radiusChecks, ratio: radiusChecks ? radiusPass / radiusChecks : 0, requiredRatio: helixRequiredRatio },
    pitchPhaseConsistency: { pass: phasePass, total: phaseChecks, ratio: phaseChecks ? phasePass / phaseChecks : 0, requiredRatio: phaseRequiredRatio },
    hubMidpointRelation: { pass: hubPass, total: hubChecks, ratio: hubChecks ? hubPass / hubChecks : 0, requiredRatio: hubRequiredRatio },
    rungOrdering: { pass: rungPass, total: rungChecks, ratio: rungChecks ? rungPass / rungChecks : 0, requiredRatio: rungRequiredRatio },
  ...
  const allPass = Object.values(metrics).every((m) => (m.total === 1 ? m.pass === 1 : m.ratio >= (m.requiredRatio ?? topologyMinRatio)));
  ...
  const results = {
    ...
    summary: {
      ...
      criteria: {
        ...
        topologyAndRoutingMinRatioUnzipped: 0.1,
